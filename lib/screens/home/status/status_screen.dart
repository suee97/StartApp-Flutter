...
class _StatusScreenState extends State<StatusScreen> {
  ...
  Scaffold build(BuildContext context) {
    ...
    builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.data == StatusCode.SUCCESS) {
          return statusCard(studentInfo[4], studentInfo[0], studentInfo[1], studentInfo[2], studentInfo[3]);
  ...
  Widget statusCard(String paid, String name, String studentNo, String group, String dep) {
    return Container(
      ...
      child: Stack(
        children: [
          paid == "학생회비 납부자" ? SvgPicture.asset(
    ...
    try {
      ...
      if (resData["status"] == 200) {
        ...
        for (var e in data) {
          ...
          if(e["memberShip"]){
            _studentInfo.add("학생회비 납부자");
          }else{
            _studentInfo.add("학생회비 미납부자");
          }
        }
